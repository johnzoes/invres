                ...
                <div class="mb-6">
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <h1 class="text-2xl font-light text-white">
                            {{ isset($categoriaActual) ? 'Items de ' . $categoriaActual->nombre_categoria : 'Todos los Items' }}
                        </h1>
                        ...
                        @can('crear items')
                            <a href="{{ isset($categoriaActual) ? route('items.create', ['categoria' => $categoriaActual->id]) : route('items.create') }}" 
                               class="inline-flex items-center gap-2 bg-green-500/90 hover:bg-green-500 text-white font-medium px-6 py-2.5 rounded-xl transition-all duration-200 shadow-lg shadow-green-500/20">
                                ...
                                {{ isset($categoriaActual) ? 'Agregar ' . $categoriaActual->nombre_categoria : 'Nuevo Item' }}
                            </a>
                        @endcan
                    </div>
                </div>
                                ...
                                <tr class="hover:bg-gray-700/30 transition-all duration-200">
                                    ...
                                    <td class="py-4 px-6">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <button onclick="if(confirm('¿Estás seguro de eliminar este item?')) document.getElementById('delete-form-{{ $item->id }}').submit();"
                    ...
                    @forelse($items as $item)
                        <div class="bg-gray-700/30 rounded-xl p-4 border border-gray-700/50">
                            ...
                            <div class="mt-4 flex justify-end gap-2">
                                ...
                                <button onclick="if(confirm('¿Estás seguro de eliminar este item?')) document.getElementById('delete-form-mobile-{{ $item->id }}').submit();"
                                        class="p-2 text-red-400 hover:bg-red-500/10 rounded-full transition-all">
                                    <x-icon name="delete"></x-icon>
                                </button>
                            </div>
    ...
    <script>
        function handleSearch(event) {
            ...
            fetch(`{{ route('items.search') }}?q=${encodeURIComponent(query)}`)
                ...
                .then(data => {
                    document.getElementById('itemsTable').innerHTML = data.html;
                })
